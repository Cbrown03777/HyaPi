...
setHttpJSONImpl(async (url, init) => {
  if (typeof url === 'string' && url.includes('aave.com')) {
    return { data: { reserves: [{ symbol: 'USDT', liquidityRate: String(0.05 * 1e27) }] } } as any;
  }
  if (typeof url === 'string' && url.includes('openapi.just.network')) {
    return { code: 0, message: 'ok', data: { tokenList: [{ symbol: 'USDT', supplyApy: '6.5', miningSupplyApy: '1.2' }] } } as any;
  }
  return {} as any;
});
